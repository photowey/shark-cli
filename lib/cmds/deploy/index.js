...
async function deploy(config = {}) {
    ...
    addtionalMakeUp(answers, config)
    ...
    if (answers['release']) {
        ...
        await releaseProject(releaseScript)
    }
 ...
 * @param {*} config 
 */
function addtionalMakeUp(answers, config) {
    if (!answers['branch']) {
        answers['branch'] = config['branch']
    }
    if (!answers['evn']) {
        answers['evn'] = config['evn']
    }
    if (!answers['machine']) {
        answers['machine'] = config['machine']
    }
}
...
async function releaseProject(releaseScript) {
